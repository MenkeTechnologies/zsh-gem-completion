#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-gem-completion — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-gem-completion.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-gem-completion.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-gem-completion.plugin.zsh"
    assert $state equals 0
}
